...
export default function AddItemCard({ t, viewMode = 'grid' }) {
    ...
    return (
        ...
        >
            <div className="relative aspect-square w-full rounded-xl border-2 border-dashed border-gray-300 bg-gray-50 hover:bg-primary/5 hover:border-primary transition-all flex items-center justify-center">
                <div className="w-16 h-16 rounded-full bg-white border border-gray-200 flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm">
                    <Plus className="text-gray-400 group-hover:text-primary transition-colors" size={32} />
                </div>
            </div>

            <div className="flex flex-col items-center text-center pt-2">
                <h3 className="font-bold text-gray-500 group-hover:text-primary transition-colors text-base">{t.addItem}</h3>
                <p className="text-xs text-muted-foreground">{t.tapToAdd}</p>
            </div>
        </motion.div>
    );
}
